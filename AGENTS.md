...
### Compile diagnostics (the "logs to console only" gap)
The in-page adapter can only say "failed to compile to blocks": MakeCode logs the real TS errors (`main.ts(L,C): error TS####: ...`) to the editor iframe's console, which is cross-origin. Both targets enrich that error with the actual diagnostics; the gate regex and `Compiler errors:` format live in `shared/compile-errors.ts` so the paths can't drift, and only gate-matching failures are enriched (transport errors never pick up diagnostics). Parity is typecheck errors: valid TS that merely can't decompile to blocks yields no diagnostics on either target.

**Server: console scrape via CDP.** `adaptPuppeteerBrowser` instruments each page in place (idempotent via WeakSet, not wrapped, so `openWindow`'s identity-based set-difference still holds), feeding console output into a per-page, time-windowed `MakeCodeDiagnostics` buffer exposed as `PageLike.recentDiagnostics(withinMs)`; `PuppeteerDriver.setProject` reads it on gated failures. Gated test: `makecode-diagnostics.puppeteer.test.ts`.

**Browser: local recompute via pxt-mkc** (`browser/local-compiler.ts` + `browser/mkc-host.ts`, called from `IframeExecutor.setCode`). Reading the editor's own errors from the app is a verified dead end (July 2026): the console is cross-origin, and pxt's `workspacediagnostics` event is gated on `allowParentController`, which makecode.microbit.org doesn't set; don't re-investigate. Instead pxt-mkc downloads the live editor's own compiler (`pxtworker.js` + `target.json`, from the same site the iframe embeds, so versions track; update check at most once daily) and runs it in a web worker. Gotchas:

- Lazy singleton: the ~4.6 MB download happens on the first compile failure (persisted via the Cache API); warm recompiles are <100 ms. Calls are mutex-serialised (one shared project) and **fail open**, returning `[]` so the original error always survives.
...
- Deps pinned exact (`makecode-core` 1.7.9, `makecode-browser` 1.3.5); pxt-mkc master has `require("vm")`, so audit `built/*.js` for Node builtins before bumping. Every bundler consuming `makecode-mcp/browser` needs `resolve.alias { path: "path-browserify" }` (app and test-page vite configs); a missing alias fails **open**, silently dropping diagnostics, so the e2e spec asserts the `Compiler errors:` block renders.
- DevTools: everything here logs as `[mkcp:local-compiler]` (fetch traces, `wireMkcLogging` redirect, collapsed groups); iframe errors stay unprefixed under the makecode frame. Known exception: mkc's downloader prints bare `Download <url>` lines via a module-private `console.log` (cold init only; one-line upstream fix candidate). Gated test: `local-compiler.puppeteer.test.ts` (production vite build + live editor).
