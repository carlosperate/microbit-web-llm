...
### Server-only compile diagnostics (the "logs to console only" gap)
That generic "failed to compile to blocks" message is all the in-page adapter can produce: MakeCode logs the real TS errors (`main.ts(L,C): error TS####: ...`) to the editor iframe's console, and the iframe is cross-origin, so neither the shim nor the browser app can read them. The server can, because CDP `page.on('console')` sits below the same-origin boundary. So on the **server target only**, the actual diagnostics get appended to the error (`...\n\nCompiler errors:\n<lines>`), giving the model line/column messages to fix. This is why the app and MCP genuinely differ here, despite sharing the adapter.

Wiring: `adaptPuppeteerBrowser` instruments each page in place (idempotent via WeakSet, not wrapped, so `openWindow`'s identity-based set-difference still holds), feeding its console into a per-page `MakeCodeDiagnostics` buffer (`makecode-diagnostics.ts`, time-windowed so stale errors from an earlier program don't attach) exposed as `PageLike.recentDiagnostics(withinMs)`. `PuppeteerDriver.setProject` reads it only when the failure message matches `/failed to compile to blocks/i`, so transport errors don't get diagnostics stapled on. Adding compile-error surfacing to the *shared* adapter can't work; it has no console access. Real-MakeCode coverage is the gated `makecode-diagnostics.puppeteer.test.ts`.
